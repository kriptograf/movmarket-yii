...
{
	/**
	 * @var boolean  Использование уменьшенная версия gmap3 библиотеки, по умолчанию true.
	 */
	public $mini = true;
	/**
	 * @var integer Ширина виджет карты. По умолчанию используется "пикселах", 
	 * но и другие единицы могут быть использованы.
	 */
	public $width = 550;
	/**
	 * @var integer Высота виджет карты. По умолчанию используется "пикселах", 
	 * но и другие единицы могут быть использованы.
	 */
	public $height = 400;
	/**
	 * @var string CSS единицы для использования, такими как : 'px', 'em', 'pt', etc ..
	 */
	public $unit = 'px';
	/**
	 * @var boolean Установите виджет карты будет изменять размер, по умолчанию является false.
	 */
	public $resizable = false;
	/**
	 * @var boolean Показать слой, который отображает велосипедных дорожек 
	 * и тропинок и понижает большой дороги.
	 */
	public $bicyclingLayer;
	/**
	 * @var boolean Показать слой, который отображает текущее дорожное движение 
	 */
	public $trafficLayer;
	/**
	 * @var boolean Масштаб карты автоматически соответствовать содержащихся в нем элементов.
	 */
	public $autofit;
	//
	...
	/**
	 * Обновление маркер на карте с помощью полей модели в Yii. 
	 * поля Модель должны существовать на этой странице.
	 *
	 * @param CModel $model Model to use
	 * @param array $attributes Model атрибутов, используемых для построения адрес.
	 * Должны быть даны в правильном порядке !
	 * @param array $options Options to set :<ul>
	 * <li>'first' - установлен в первый маркер добавляется к карте, по умолчанию это последний
	 * <li>'nopan' - Не перемещения по карте маркером на обновления.
	 * <li>'event' => 'change' - действительный JQuery событием для запуска обновления карты.
	 * </ul>
	 */
	public function updateMarkerAddressFromModel(CModel $model, array $attributes, array $options=array())
	{
		// получить идентификаторы атрибут 
		for ($i = 0; $i < count($attributes); $i++) {
			$attributes[$i] = CHtml::activeId($model, $attributes[$i]);
		}
		// создания скрытого поля
		...
		// Построим положение маркера обновления функция
		...
		// Построим поле адреса обновления функция
